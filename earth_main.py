...
ax.bar(range(len(half_lives)), half_lives, color='blue', label='Half-life')
...
# 선택된 동위원소의 반감기 강조
ax.bar(selected_idx, selected_half_life, color='orange', label=f'{labels["annotate_selected"]} {selected_isotope}')
...
ax.set_title(labels['bar_chart_title'])
...
if st.button('더보기'):
    ...
    ax.bar([nearest_idx, selected_idx], [half_lives[nearest_idx], selected_half_life], color=['green', 'orange'], label=[nearest_isotope, selected_isotope])
